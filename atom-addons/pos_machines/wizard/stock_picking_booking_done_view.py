...
class BookingDonePickingLine(models.TransientModel):
    _name = "stock.booking.picking.line"
    ...
    wizard_id = fields.Many2one('stock.booking.picking', string="Wizard")
    move_id = fields.Many2one('stock.move', "Move")


class BookingPicking(models.TransientModel):
    _name = 'stock.booking.picking'
    ...
    product_return_moves = fields.One2many('stock.booking.picking.line', 'wizard_id', 'Moves')
    ...
    def _onchange_picking_id(self):
        ...
        line_fields = [f for f in self.env['stock.booking.picking.line']._fields.keys()]
        product_return_moves_data_tmpl = self.env['stock.booking.picking.line'].default_get(line_fields)
    ...
    def _prepare_move_default_values(self, booking_line, new_picking):
        vals = {
            'product_id': booking_line.product_id.id,
            'product_uom_qty': booking_line.quantity,
            'product_uom': booking_line.product_id.uom_id.id,
            'picking_id': new_picking.id,
            'state': 'draft',
            'date': fields.Datetime.now(),
            'location_id': booking_line.move_id.location_dest_id.id,
            'location_dest_id': self.location_id.id or booking_line.move_id.location_id.id,
            'picking_type_id': new_picking.picking_type_id.id,
            'warehouse_id': self.picking_id.picking_type_id.warehouse_id.id,
            'origin_returned_move_id': booking_line.move_id.id,
    ...
    def _create_out_picking(self):
        ...
        returned_lines = 0
        for booking_line in self.product_return_moves:
            if not booking_line.move_id:
                raise UserError(_("You have manually created product lines, please delete them to proceed."))
            # TODO sle: float_is_zero?
            if booking_line.quantity:
                returned_lines += 1
                vals = self._prepare_move_default_values(booking_line, new_picking)
                r = booking_line.move_id.copy(vals)
                vals = {}

                # +--------------------------------------------------------------------------------------------------------+
                # |       picking_pick     <--Move Orig--    picking_pack     --Move Dest-->   picking_ship
                # |              | returned_move_ids              ↑                                  | returned_move_ids
                # |              ↓                                | booking_line.move_id              ↓
                # |       return pick(Add as dest)          return toLink                    return ship(Add as orig)
                # +--------------------------------------------------------------------------------------------------------+
                move_orig_to_link = booking_line.move_id.move_dest_ids.mapped('returned_move_ids')
                # link to original move
                move_orig_to_link |= booking_line.move_id
                ...
                move_dest_to_link = booking_line.move_id.move_orig_ids.mapped('returned_move_ids')
                # link to children of originally returned moves, if any. Note that the use of
                # 'booking_line.move_id.move_orig_ids.returned_move_ids.move_orig_ids.move_dest_ids'
                # instead of 'booking_line.move_id.move_orig_ids.move_dest_ids' prevents linking a
    ...
    def create_out_picking(self):
        for wizard in self:
            new_picking_id, pick_type_id = wizard._create_out_picking()
